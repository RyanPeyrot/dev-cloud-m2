...
test('No render if not any completed tasks', () => {
  const taskList = [{id: 1, text: 'Task 1', completed: false}];
  ...
  const completedTasksElement =
    renderWithTasksContext(taskList).getByRole('list');
  expect(completedTasksElement).toBeEmptyDOMElement();
});

test('Completed tasks are rendered', () => {
  const taskList = [{id: 3, text: 'Task 3', completed: true}];
  ...
  const completedTasksElement =
    renderWithTasksContext(taskList).getAllByRole('listitem');
